...
router.put("/afternoonjobstop/:tabid", async (req, res) => {
  try {
    const tabId = req.params.tabid;
    if (!tabId) {
      console.log("No tablet id found");
    ...
    afternoonTask.stop(); //stopped the afternoon task
    const tabdb = await Tablet.findByIdAndUpdate(
      tabId,
      {
        $set:
...
router.put("/eveningjobstop/:tabid",async(req,res)=>{
  try{
    ...
    eveningTask.stop(); //stopped the evening task
    const tabdb = await Tablet.findByIdAndUpdate(
      tabId,
      {
        $set:
